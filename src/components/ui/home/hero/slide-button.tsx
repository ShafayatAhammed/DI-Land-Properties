...
const SlideButton: FunctionComponent<SlideButtonProps> = ({
  ...
  return (
    <button
      type="button"
      className="w-[50px] h-[50px] xl-2xl:w-[60px] xl-2xl:h-[60px] border-2 border-white rounded-full flex justify-center items-center cursor-pointer hover:bg-white hover:fill-blue-1 transition-colors fill-white outline-none"
      onClick={() => {
        if (name === "next") {
    ...
    >
      {(name === "prev" && (
        <svg
          ...
          width={30}
          height={30}
          className="xl-2xl:w-[30px] xl-2xl:h-[30px] w-[25px] h-[25px]"
        >
